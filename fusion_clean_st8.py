...
LLM + BERT 实体融合 (重写整洁版)
========================================
- 数据解析 / 候选生成 / 评估: 与 gating_network_dp_mark_v3.py 保持完全一致
- 实现两种融合策略:
...
# Python hash 随机化关掉, 避免 dict 顺序抖动影响结果
...
# 位置容差: LLM/BERT 预测位置 |diff| <= TOL 视为同一实体, label 匹配同样使用
POSITION_TOLERANCE = 2

# 评估时使用 startIndexMatch: key = (start_idx, entity_text, type)
# 这里的 start_idx 用 int(e['start_idx']) 的精确值, 不再容差 (与原版 evaluate_entities_with_tolerance 一致)


# ====================================================================
# 1. 数据解析 (与原文件完全一致, 略作精简)
# ====================================================================
...
def _is_mark_format_line(line: str) -> bool:
    """检测单行是否为 mark 格式 (predict 字段包含 ' :' 标记)
    严格的 mark 格式特征: 行内有 ' :' 且在方括号两侧 (表示实体标注),
    而非 JSON 数组碰巧含冒号/方括号。

    关键: 不能只看 models[0].predict — 5-beam 数据中 models[0].predict 可能为空
    (LLM 服务异常重试) 而 models[1..4] 才是 mark 格式; 此时若误判为标准 JSON,
    parse_llm_line 会走 JSON 解析路径, 把 mark 格式的 label 当 JSON 解析失败 → 丢实体
    (本次 bug: train 集丢 97 个 label, 导致 LLM Only 与 LLM Beam-1 F1 不一致,
     差约 0.001-0.004, 虽小但 p<0.001 显著)。
    修正: 同时检查 (a) 任一 beam predict 含 mark 特征, (b) label 字段含 mark 特征。
    """
    ...
    try:
        ...
        # (a) 任一 beam predict 含 mark 特征
        if models:
            for m in models:
                predict = m.get('predict', '') or ''
                if ' :' in predict and bool(re.search(r'\[[^\[\]]+\]', predict)):
                    return True
        # (b) label 字段含 mark 特征 (兜底: 即使所有 predict 都被截断为空,
        #    只要 label 字段仍是 mark 格式, 也应走 mark 解析路径)
...
def _split_mark_line(line):
    ...
    (与 llm_ner_eval_Temp_mark.py 保持一致)"""
    line = line.strip()
...
def _entities_with_pos_in_content(content):
    ...
    返回 [(start, end, entity), ...]. entity 始终 strip 掉首尾空白, 以便与 LABEL 的 [entity] 形式能匹配.

    与 llm_ner_eval_Temp_mark.py._entities_with_pos_in_content 完全一致。
    """
    out = []
...
def parse_marked_text_with_pos(text: str):
    ...
    位置是剔除 [ 和 ] 后, 在该行内容中的真实索引。
    注: end_idx 是闭区间 (start + len(entity) - 1), 与标准 JSON 格式的
       开区间 end_idx (= start + len(entity)) 不同, 但 build_candidates
       只用 (start_idx, entity, type) 三元组作 key, 故不影响匹配。

    兼容 [entity] 标记与裸实体两种格式 (与 llm_ner_eval_Temp_mark.py 保持一致)。
    裸实体原本会被漏掉, 导致 LLM Beam-1 F1 偏低约 25 倍 (0.0117 vs 0.2854)。"""
    result = []
...
def _correct_entity_positions(entities_with_pos, original_text, tolerance=2,
    ...
    校正规则 (与 llm_ner_eval_Temp_mark.py._correct_to_original_text 完全一致):
      1. 实体不在 original_text 中 → 丢弃 (LLM 幻觉, 不参与评估)
      2. 实体在 original_text 中, 位置已正确 → 保留
      3. 实体在 original_text 中, 位置不匹配:
         a. 仅 1 处匹配 → 直接使用该位置
         b. 多处匹配:
            - keep_all_matches=False (默认): 启用位置容差 `tolerance` (默认 2),
              优先选 |match.start - 预测 start| <= tolerance 中距离最近的位置;
              若无容差内匹配, 退而选全部匹配中距离最近的位置
            - keep_all_matches=True: 全部采用, 把每一处匹配都作为候选位置加入

    原版 (无容差、无 keep_all_matches、无幻觉剔除) 在 LLM 大量输出裸实体 + 实体多次出现时
    会: (a) 把不在原文本的实体当 TP/FP 算; (b) 永远 re.search 首处位置 → indexMatch F1
    偏低。升级为带容差版本后, 与 llm_ner_eval_Temp_mark.py 的 F1 保持一致。
    """
...
# LLM 聚合参数 (max+mean+vote_reward 模型)
# 改用数据驱动的 max+mean+vote_reward 模型, 取代 hardcode 的排名权重
# 公式:  P = α · max(c_i) + (1-α) · mean(c_i) + β · (N-1)
#  - α: 最佳 beam conf 的权重 (默认 0.7)
#  - β: 投票数线性奖励 (每多 1 票加 β, 默认 0.05, 5 票最多加 0.20)
#  - N: 预测该实体的 beam 数量
...
def _aggregate_llm_conf(confs, beam_idxs, alpha, beta):
    ...
    P(correct | N beams predict it) = α · max(c_i) + (1-α) · mean(c_i) + β · (N-1)
    clamp 到 [0, 1]

    优点 (相比 Noisy-OR):
      - 不依赖 hardcode 的 rank weight, 也不依赖独立性假设
      - 5 票 conf=0.5 → 0.5 + 0.20 = 0.70 (不会虚高到 0.97)
      - 1 票 conf=0.9 → 0.9 (高 conf 单源)
      - 区分度好: 多票低 conf (0.70) < 单票高 conf (0.90)
    """
    if not confs:
        return 0.0
    a = alpha if alpha is not None else 0.7
    b = beta if beta is not None else 0.05
    # 加性 vote_reward, 配合搜索范围扩到 [-0.05, +0.10]:
    #   β>0: 票多加分 (适用于 con_th 较高的硬阈值场景, 鼓励多 beam 一致)
    #   β<0: 票多扣分 (适用于多 beam conf 都低的情况, 当作幻觉信号)
    # 经验: 在 A 策略软融合上, β 对 F1 影响 < 0.005 (N-1 加分被 max conf 主导效应掩盖)
    #       真正起作用的是 A'' 的硬规则票数门 (min_vote_llm), 而非软加性奖励
...
def _parse_mark_format_beam(models, original_text, default_conf=0.8):
    """5-beam max+mean+vote_reward 聚合 (按用户说明: beam 1 准确性最高, 5 最低)。
    输入: models = [ {predict: str, confidence: float}, ... ]  (按 beam 排名 1→5)
    输出: { (start,end,type,text): {
              confidence,        # max+mean+vote_reward 聚合分
              entity, type, start_idx, end_idx,
              vote_count,        # 多少 beam 预测了该实体 (1~5)
              best_beam_idx,     # 最准的 beam 排第几 (0=beam1, ..., 4=beam5)
              best_beam_conf,    # 最准 beam 的置信度
              beam_idxs,         # 预测该实体的所有 beam 索引列表
              beam_confs,        # 预测该实体的所有 beam 置信度列表
              llm_avg_conf,      # 平均 conf
              llm_max_conf,      # 最大 conf
           } }
    """
    if not models:
        return {}
...
def parse_llm_line(line, source='ner_models', use_beam=False, default_conf=0.8):
    ...
    use_beam=True:  对 5 beam 投票, entity_dict 中包含 vote_count / llm_avg_conf / llm_max_conf
    use_beam=False: 仅用第 1 个 beam (LLM 服务对外默认返回 1 个, 此时 c['llm_conf'] = Beam-1 真实 conf,
                    vote_count=1, 没有多 beam 投票信号)

    关键修复: 即便 models=[] (LLM 服务 CUDA OOM 等异常导致 ner_models 为空),
              也必须解析 label 作 ground truth。原版 if not models: return ... 早返回
              会丢 label, 导致 [基线] LLM Only F1 与 [基线] LLM Beam-1 F1 不一致
              (本次 bug 累积: train 集丢 82 个 label, 测试集丢 0 个)。
    """
    obj = _safe_parse_json(line)
    if obj is None:
        return None
    ...
    # 即便 models 为空, 也要把 label 解析出来作 ground truth
    ...
    if not models:
        # 早返回, 但保留 label_entities (不再丢 label)
    ...
    if _is_mark_format_line(line):
        if use_beam and len(models) >= 2:
            ...
            # 单 beam: 仍走 _parse_mark_format_beam 让 cand 携带 Beam-1 真实 conf + vote_count=1,
            # 方便后续 A / A'' / A_v2 / B 等融合函数统一接口 (无需特判 Beam-1)
    ...
    # 标准 JSON 格式
    # 注: 此处 end_idx = s + len(entity) 是开区间 (不含末位), 与 mark 格式的
    #     闭区间 end_idx 不同, 但 build_candidates 用三元组 key 故不影响匹配。
    entity_dict = {}
    for model in models:
...
def parse_bert_line(line):
    ...
    加速: 用 LRU 缓存 (key=line), 5-beam union / 多策略评估 / bert_only_predict
          等场景同一行 BERT 反复解析, 缓存命中直接返回。"""
    cached = _BERT_LINE_CACHE.get(line)
    ...
    if len(_BERT_LINE_CACHE) < 8192:  # 简单 LRU 风格上限
        _BERT_LINE_CACHE[line] = result
    return result


# BERT 行解析缓存 (key=原始行字符串)
_BERT_LINE_CACHE = {}
...
def build_candidates(text, entity_dict, bert_entities, tol=0):
    """LLM 实体 (entity_dict) + BERT 实体 → 候选列表
    每条候选: {start_idx, end_idx, type, entity, llm_conf, bert_conf,
                llm_present, bert_present,
                [可选 beam 特征] vote_count, llm_avg_conf, llm_max_conf}
    """
    cand_dict = {}
    ...
    for ent in llm_list:
        ...
        # 若有 beam 特征则携带 (仅在 use_beam=True 时存在)
        if 'vote_count' in ent:
            ...
            # 实体被哪几个 beam 预测过 (用于 LLM Top-k 复现)
            cand['llm_beam_idxs'] = list(ent.get('beam_idxs', []))
            # 原始 beam confs 列表 (Step 1.5 重算 llm_conf 用, 缺这个 → _reaggregate_llm_confs 退化为常数)
...
def _label_set(label_entities, tol=0):
    ...
    注: 与 _precompute_metrics 保持一致, start_idx 转 int 后再 + off
        (避免 build_samples 拿到的是字符串 start_idx 时 str+int 报错)
    """
    if tol <= 0:
...
def _reaggregate_llm_confs(samples, alpha, beta):
    """重新算每个 candidate 的 llm_conf = α·max + (1-α)·mean + β·(N-1)
    不重 build_samples, 直接用候选里已存的 beam_confs 重算。
    用途: Step 1.5 网格搜 (α, β) 时, 30 次只调此函数, 不重解析文本/位置 → 提速 30x。

    优先级:
      1) c['beam_confs'] (非空 list) — 真实多 beam confs
      2) c['llm_max_conf']            — 退化为单值 max (无 mean/avg 区分)
      3) c['llm_conf']                — 最后兜底 (退化为常数, (α, β) 无效果)
    """
    for s in samples:
        for c in s['candidates']:
            confs = c.get('beam_confs') or None
            if not confs:  # None / [] 都算空
                max_c = c.get('llm_max_conf')
                if isinstance(max_c, (int, float)) and max_c > 0:
                    # 退化: 用 max + 1 个当前 conf 近似 (让 vote_count 信号至少能影响)
                    confs = [float(max_c), float(c['llm_conf'])]
                else:
                    # 最后兜底: 退化为常数, (α, β) 无效果, 但不崩
...
# ====================================================================
# 3. 评估函数 (与原版 evaluate_entities_with_tolerance 完全一致)
# ====================================================================
def evaluate(preds_list, labels_list, tol=0):
    """startIndexMatch 评估: (start_idx, entity, type) 集合命中
    tol: 位置容差 (论文严谨做法: build_candidates 用 tol=POSITION_TOLERANCE=2,
          评估时也用 tol=2, 避免"匹配成功但评估失败"。
          0 = 严格精确匹配, 2 = 论文主评估。)
    """
...
def _precompute_metrics(preds_list, labels_list, tol=0):
    ...
    用途: bootstrap 1000 次重采样时, 不再重算 set 交集, 直接向量化和。
    加速: 5-10x (取决于样本数 n)。
    """
    n = len(preds_list)
...
def bootstrap_f1_ci(preds_list, labels_list, n_boot=1000, alpha=0.05,
    ...
    """Paired Bootstrap 算 F1 的 95% 置信区间 + p-value (vs 0)。

    方法: 有放回重采样文本索引 1000 次, 每次算 F1, 取 [α/2, 1-α/2] 分位数。
    返回: (mean, lo, hi, std)
    加速: 预计算 (tp, |p|, |l|) 三个数组, bootstrap 时只向量化求和, 不重算 set。
    """
...
def paired_bootstrap_pvalue(preds_a, labels_a, preds_b, labels_b,
    ...
    """Paired Bootstrap 检验 H0: F1(B) - F1(A) = 0 vs H1: != 0。
    返回: (delta_mean, delta_ci_lo, delta_ci_hi, p_value_two_sided)
    加速: 预计算 A/B 各自的 (tp, |p|, |l|), bootstrap 时向量化算 ΔF1。
    """
    rng = np.random.RandomState(seed)
    n = len(preds_a)
...
def paired_bootstrap_pvalue_from_arr(tps_a, pns_a, tns_a, tps_b, pns_b, tns_b,
    ...
    """paired_bootstrap_pvalue 的预计算数组版本, 严格评估块用 — 多策略共享预计算结果。
    """
    rng = np.random.RandomState(seed)
    n = len(tps_a)
    ...
    # 双边 p-value: H1: ΔF1 ≠ 0
    #   n_le = #{deltas <= 0}, n_gt = #{deltas > 0}
    #   p = 2 * min(n_le, n_gt) / n_boot, 再 cap 到 1.0
    # (原版用 np.abs(deltas) <= 0 → 浮点下永远 = 0, 导致 p=0.000 假阳性, 修正)
...
def bootstrap_f1_ci_from_arr(tps, pns, tns, n_boot=1000, alpha=0.05,
                              seed=GLOBAL_SEED):
    """bootstrap_f1_ci 的预计算数组版本, 严格评估块用 — 多策略共享预计算结果。
    """
    rng = np.random.RandomState(seed)
    n = len(tps)
    if n == 0:
    ...
    idx_buf = np.empty(n, dtype=np.int64)
    for b in range(n_boot):
        # 兼容 numpy 新版本: 不再支持 randint 的 out 参数
        idx_buf[:] = rng.randint(0, n, size=n)
        tp = tps[idx_buf].sum()
        pn = pns[idx_buf].sum()
        tn = tns[idx_buf].sum()
...
def eval_llm_beam_k(llm_lines, bert_lines, beam_idx=0, default_conf=0.8,
    ...
    输入: llm_lines, bert_lines (与 build_samples 同源, 都是 JSON 字符串列表)
    返回: (P, R, F1, TP, pred_n, true_n)  ← 与 evaluate 签名一致
    关键: ground truth 必须用 LLM 文件的 label 字段 (人工标注), 不是 BERT 文件的 entities
          (BERT entities 是 BERT 模型的推理结果, 不是标注)。
    """
    preds, labels = [], []
    for ll, bl in zip(llm_lines, bert_lines):
        ...
        if beam_idx is not None and isinstance(beam_idx, int) and beam_idx < len(models):
            ...
            predict_str = ''  # beam_idx=None/越界 → 5-union 由 main 另一函数处理
        ...
        # ground truth 用 LLM 文件的 label 字段 (mark 格式, 人工标注)
...
def build_samples(llm_lines, bert_lines, source='ner_models', tol=POSITION_TOLERANCE,
    ...
    for i, (ll, bl) in enumerate(zip(llm_lines, bert_lines)):
        ...
        if llm_err:
            entity_dict = {}
        # label 统一用 LLM 文件 label 字段 (mark 格式, 实际人工标注 ground truth)
        # BERT 文件的 entities 字段是 BERT 模型的推理结果, 不是 ground truth, 不可作为评估基准。
        # 之前误用 BERT entities 作 ground truth 导致 F1 虚高, 改回 LLM label 字段。
        # label_entities 来自 parse_llm_line → _parse_mark_format_label → parse_marked_text_with_pos
        # 坐标系: inclusive 闭区间 (start, end=start+len-1), 统一用 _correct_entity_positions 校正到原文本
        ...
        # 兼容: 如果 label_entities 已经是 dict 列表, 直接转 tuple 形式
...
def bert_only_predict(samples, llm_lines, bert_lines, source='ner_models'):
    ...
    preds_list, labels_list = [], []
    for s, ll, bl in zip(samples, llm_lines, bert_lines):
        _, bert_ents = parse_bert_line(bl)
        # 取 start_idx/entity/type 三元组对应的实体 dict
...
def llm_only_predict(samples, llm_lines=None, bert_lines=None,
    ...
    注意: LLM 服务对外只返回 1 个最佳 (Top-1) 答案, 因此
        - 论文主基线 = llm_only_predict(..., beam_idx=0)
        - 论文 ablation: llm_only_predict(..., beam_idx=None) (5-union)
    """
    preds_list, labels_list = [], []
    for s in samples:
        kept = []
        for c in s['candidates']:
            if not c['llm_present']:
                continue
            # beam 过滤: 还原"单 beam 预测集" = 实体在指定 beam 出现过
            if beam_idx is not None:
                idxs = c.get('llm_beam_idxs', [int(c.get('best_beam_idx', 0))])
                if beam_idx not in idxs:
                    continue
            # conf 过滤 (用 best_beam_conf, 因为这是该实体最准 beam 的 conf)
...
def _consensus_stats(samples):
    """统计: 共识 (BERT+LLM 同预测) 的精确率 / 单独预测的精确率。这是策略 A 共识加成的依据。"""
    both_p = both_t = 0
    bert_p = bert_t = 0
    llm_p = llm_t = 0
    for s in samples:
...
def static_fusion_predict(samples, type_alphas, type_thresholds,
    ...
    核心思想: 共识实体的精度远高于单源预测, 应当显著加分并降低阈值。
        both-present: score = α*bert + (1-α)*llm + bonus + vote_bonus_coef * log(vc)/log(5),  th *= consensus_th_mult
                      (vote_bonus: 5/5 票再多加 ~0.05, 1/5 票不加)
...
# ====================================================================
# 6. 策略B: 门控网络融合 (Gating Network Fusion) — 输出两路权重
# 关键修复:
#   - 网络输出 (w_bert, w_llm) 两路 sigmoid 权重 (取代旧的单路 g)
...
# ====================================================================
# 策略 A_v2: Consensus V2 Hard-Rule
# ====================================================================
# 共识细分 (per-beam 特征)
...
def run_a_v2(train_samples, test_samples):
    ...
    # 返回 (val_f1, test_f1, bsolo_th, lsolo_th, mv_th, sp_th, stats)
    # mv_th/sp_th 是网格搜出的 per-type 共识票数门/分散度门, main 汇总表必须用, 不能 hardcoded
...
# ====================================================================
# 思路 (移植自 gating_network_dp_mark_v3.py 策略3/9, 适配 st8 架构):
#   1) 在训练集上用 sklearn IsotonicRegression 分别拟合
#        bert_cal: conf → P(正确|bert_conf)
#        llm_cal:  conf → P(正确|llm_conf)
#      校准后两个 conf 处于同一可比尺度, 解决 LLM conf 偏高/集中导致的加权偏差
#      (典型: LLM conf 全在 0.7~0.8, BERT conf 偏低且分散, 直接加权会被 LLM 主导)
#   2) 简单 per-type α 加权 + 全局阈值:
#        score = α(类型) * cal_bert + (1-α) * cal_llm
#        keep if score >= th
#   3) 全局网格搜 (α, th) → per-type α 微调
# 与现有 A 的关键区别:
#   - A 用 3-case (共识/bert-only/llm-only) + 共识加成, 4 个全局超参
#   - A_cal 用纯 2-特征加权, 只搜 (α, th), 与 v3 策略 3/9 思路一致
#   - A_cal 多了 isotonic 校准, 在 conf 尺度不一致时 (LLM 全在 0.7~0.8)
#     解决"被 LLM 主导"问题
# 关键差异 (来自 v3 消融):
#   - 校准本身贡献仅 +0.0002 (v3 log 末尾: "校准贡献 = +0.0002"),
#     主要提升来自 per-type α 加权 (v3 log: 验证 F1 由 0.7509 提到 0.7533)
#   - 5-beam / Beam-1 都可应用此策略, 校准用各自训练集重 fit
#   - 校准会改 cand['bert_conf'/'llm_conf'] in-place, 故 deep copy 后再校准
#     避免污染原 train_samples / test_samples (供 A/A''/A_v2/B 复用)
# ====================================================================
def _fit_isotonic_calibrators(samples):
    ...
    重要: 若某模型正例率 = 100% 或 = 0%, sklearn IsotonicRegression 报错
          (只一个 class), 此时 cal.f_ 仍为 None, 上层 predict 走 fast-path 不校准。
    """
...
def _calibrate_samples_inplace(samples, bert_cal, llm_cal):
    ...
    防呆: cal.f_ is None (单 class 未拟合) 时跳过校准, 保持原 conf 不变。
    """
    if not samples:
        return
    for s in samples:
...
def run_calibrated_weighted_fusion(train_samples, test_samples, label='A_cal'):
    ...
    返回: (val_f1, test_f1, type_alphas, threshold, train_cal, test_cal, cal_info)
      - train_cal / test_cal: 校准后的 deep copy, 供 strategy_preds / 汇总表复用
        (避免 A/A''/A_v2/B 拿到被校准改写的 conf 分布)
    """
    ...
    print("  4) 单一阈值 th (无共识加成, 无 bert/llm_only 因子) ← 与 v3 策略3/9 一致")
    print("-" * 70)
    ...
    # 用未校准的 train_samples / test_samples 跑同样 (α, th) + per-type α,
    # 量化"校准本身" vs "per-type α 加权" 各自的贡献
...
def _predict_with_calibrators_reuse_alphas(target_samples, train_for_fit,
    ...
    关键: 每次调用都重 fit 校准器 (target_samples 不能复用原 train_cal 的校准器,
          因为 train_cal 是 deep copy 后的, 若原函数 A_cal_alphas/th 选自 5b
          训练, va_samples 也是 5b 解析, 校准器尺度仍一致; 重新 fit 不会改变
          分布规律, 校准器等价; 真正变化是 acal_alphas/th 是固定超参
          — 它们不依赖校准器, 只依赖样本 conf 分布)
    简化: 严格块我们用 '重 fit + 复用 alphas/th' 的安全做法, 不留隐患。
    返回: preds (list of list of dict), 与 _cal_fusion_predict 同步签名
    """
    import copy as _copy
    if train_for_fit is None or len(train_for_fit) == 0:
        # fallback: target_samples 直接 predict (无校准)
...
def main():
    ...
    if bb1_va is not None:
        # 重新在 bb1_va 上 fit 校准 + 评估? 不, 严格评估块用现成 train_cal_b1 / test_cal_b1 上的
        # acalb1_alphas / acalb1_th 在 va_samples_b1 / test_samples_b1 上重算 (校准已 fit 过, 只重算 score)
        # 但 va_samples_b1 仍是未校准的, 需要重校准: 直接在 train_cal_b1 同样的 calibrator 基础上
        # 重新对 va_samples_b1 校准更准确。简化: 这里只把汇总指标算出来, 严格评估块用 acalb1_alphas/th
        # 在 test_cal_b1 上重算 (已经校准好), val 列同样用现成 acalb1_vf1 (in-sample)
        pass
    ...
    print(f"  {'策略':<22} | {'val F1':>7} {'95% CI':<17} | {'test F1':>7} {'95% CI':<17} | {'Δ test':>8}")
    print("  " + "-" * 90)
    strict_results = {}
    for name, (vp, tp) in strategy_preds.items():
        ...
        print(f"  {name:<22} | {fv:>7.4f} [{lov:>5.3f}, {hiv:>5.3f}] | "
              f"{ft:>7.4f} [{lot:>5.3f}, {hit_:>5.3f}] | {delta:>+8.4f}")
    ...
    for name, (vp, tp) in strategy_preds.items():
        ...
        strict_results[name] = (pv, rv, fv, pt, rt, ft, mv, lov, hiv, mt, lot, hit_)
        delta = ft - strict_results.get('BERT Only', (0, 0, 0, 0, 0, 0))[5]
        # 修改后的打印：val 部分 P / R / F1 + CI，test 部分 P / R / F1 + CI
